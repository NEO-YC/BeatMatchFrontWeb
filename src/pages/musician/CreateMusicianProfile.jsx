...
export default function CreateMusicianProfile() {
  ...
  const handleChange = (e) => {
    ...
    // אם זה שדה טלפון - מלא אוטומטית את WhatsApp (אם לא ערכו ידנית)
    if (name === 'phone') {
      setForm(prev => {
        const cleanPhone = value.replace(/\D/g, '');
        let whatsappLink = prev.whatsappLink;
        
        // רק אם השדה ריק או תואם לתבנית האוטומטית - עדכן אוטומטית
        const prevClean = prev.phone.replace(/\D/g, '');
        const prevExpected = prevClean.startsWith('0') 
          ? `https://wa.me/972${prevClean.slice(1)}` 
          : `https://wa.me/${prevClean}`;
        
        if (!prev.whatsappLink || prev.whatsappLink === prevExpected) {
          if (cleanPhone.length > 0) {
            const intlPhone = cleanPhone.startsWith('0') 
              ? '972' + cleanPhone.slice(1) 
              : cleanPhone;
            whatsappLink = `https://wa.me/${intlPhone}`;
          } else {
            whatsappLink = '';
          }
        }
        
        return { ...prev, phone: value, whatsappLink };
      });
      return;
    }
  ...
  useEffect(() => {
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        ...
        // עדכון תפריט המשתמש ומעבר חלק הביתה
      ...
      <div className="auth-container" style={{background:'linear-gradient(135deg,#f0f4ff, #fbf7ff)'}}>
        <div className="auth-card" style={{maxWidth:800}} dir="rtl">
          <div className="auth-header">
            <h1 className="auth-title" style={{background:'none',WebkitTextFillColor:'#333',color:'#333'}}>יצירת פרופיל מוזיקאי</h1>
            <p className="auth-subtitle">מלא/י את הפרטים כדי להופיע בחיפושים</p>
          </div>
          ...
          <div className="profile-hint">לחץ על העיגול כדי להוסיף תמונת פרופיל</div>
            ...
            <div className="form-row">
              ...
              <div className="form-group">
                <label className="form-label">
                  קישור לוואטסאפ *
                  <span style={{fontSize: '0.8rem', color: '#64748b', marginRight: 6}}>(ממולא אוטומטית)</span>
                </label>
                <input 
                  name="whatsappLink" 
                  type="url" 
                ...
                {images.length > 0 && (
                  <div style={{marginTop: '12px', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: '8px'}}>
                    {images.map((img, index) => (
                      <div key={index} style={{position: 'relative', aspectRatio: '1', borderRadius: '8px', overflow: 'hidden', border: '1px solid #ddd'}}>
                        <img
                ...
                {videos.length > 0 && (
                  <div style={{marginTop: '12px', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '8px'}}>
                    {videos.map((vid, index) => (
                      <div key={index} style={{position: 'relative', aspectRatio: '16/9', borderRadius: '8px', overflow: 'hidden', border: '1px solid #ddd', background: '#000'}}>
                        <video
